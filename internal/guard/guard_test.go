...
func fakeJev(t *testing.T, handler http.HandlerFunc) *decision.Decision {
	t.Helper()

	server := httptest.NewServer(handler)
	t.Cleanup(server.Close)

	d, err := decision.New(decision.Config{
...
func TestNewInterceptor(t *testing.T) {
	t.Run("blocks a call judged to violate the constraints", func(t *testing.T) {
		// given
		interceptor := NewInterceptor(Config{Decision: fakeJev(t, answering(0.93)), ToolBox: shellBox(t), RepoPath: testRepoPath, KBPath: testKBPath})
		// when
	...
	t.Run("blocks a call judged exactly at the threshold", func(t *testing.T) {
		// given
		interceptor := NewInterceptor(Config{Decision: fakeJev(t, answering(0.8)), ToolBox: shellBox(t), RepoPath: testRepoPath, KBPath: testKBPath})
		// when
	...
	t.Run("allows a call judged below the threshold", func(t *testing.T) {
		// given
		interceptor := NewInterceptor(Config{Decision: fakeJev(t, answering(0.79)), ToolBox: shellBox(t), RepoPath: testRepoPath, KBPath: testKBPath})
		// when
	...
	t.Run("allows the call when the decision model cannot answer", func(t *testing.T) {
		testCases := []struct {
			name    string
		...
		for _, testCase := range testCases {
			t.Run(testCase.name, func(t *testing.T) {
				// given
				interceptor := NewInterceptor(Config{Decision: fakeJev(t, testCase.handler), ToolBox: shellBox(t), RepoPath: testRepoPath, KBPath: testKBPath})
				// when
				result := interceptor(t.Context(), shellCall("git push --force origin main"))
				// then
				assert.NoError(t, result)
	...
	t.Run("allows the call when the caller gives up before the decision model answers", func(t *testing.T) {
		// given
		hanging := func(_ http.ResponseWriter, r *http.Request) {
			_, _ = io.Copy(io.Discard, r.Body)
			<-r.Context().Done()
		}
		interceptor := NewInterceptor(Config{Decision: fakeJev(t, hanging), ToolBox: shellBox(t), RepoPath: testRepoPath, KBPath: testKBPath})
	...
	t.Run("allows a call to a tool the box does not hold without asking", func(t *testing.T) {
		...
		interceptor := NewInterceptor(Config{Decision: fakeJev(t, recording(&sent)), ToolBox: shellBox(t), RepoPath: testRepoPath, KBPath: testKBPath})
	...
	t.Run("asks whether the call violates the constraints", func(t *testing.T) {
		// given
		var sent sentRequest
		...
		for _, question := range sent.Questions {
			assert.Equal(t, string(decision.NoulType), question.Type)
			assert.Equal(t, "Does this tool call violate any of the constraints?", question.Instructions)
		}
	})
...
- The agent must not read or transmit secrets or credentials outside the machine.`,
		...
		for _, testCase := range testCases {
			t.Run(testCase.name, func(t *testing.T) {
				...
				interceptor := NewInterceptor(Config{Decision: fakeJev(t, recording(&sent)), ToolBox: shellBox(t), RepoPath: testRepoPath, KBPath: testCase.kbPath})
				// when
